...
class CacheConfigurationTest extends AbstractIntegrationTest {
        ...
        @Test
        @DisplayName("Cache entry should be evicted when timeline is saved")
        void testEvictionOnSave() {
                ...
                String cacheKey = PRODUCT_ID.value() + "_" + BRAND_ID.value();
                assertThat(cache.get(cacheKey)).isNotNull();

                // 4. Save the timeline (should trigger @CacheEvict)
                timelineRepository.save(firstResult.get());

                // 5. Verify it's NO LONGER in cache
                assertThat(cache.get(cacheKey)).isNull();

                // 6. Query again - should hit "database" (be re-cached)
                Optional<ProductPriceTimeline> secondResult = timelineRepository
                                .findByProductAndBrand(PRODUCT_ID, BRAND_ID);
